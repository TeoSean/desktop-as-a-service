...
import errors.auth.login
import errors.auth.generic
import errors.auth.registration
...
class AuthenticationManager:
    ...
    def __init__(
        ...
        uauth_response: flask.Response = None,
    ) -> None:
    ...
    def create_session(self, uid: str,
    ...
    def login(
        ...
        cur = self.db_conn.cursor()
        cur.execute(
            "SELECT id, username, hash, salt FROM users WHERE username=%s", (
                username,)
        )
        user = cur.fetchone()
        if user is None:
        ...
        cur.execute("DELETE FROM sessions WHERE expiry < %s",
                    (int(time.time()),))
    ...
    def change_password(self, uid: str, old_password: str,
        ...
        cur.execute(
            "UPDATE users SET hash=%s, salt=%s WHERE id=%s", (phash, salt, uid))
